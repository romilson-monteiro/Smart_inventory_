...
videos = [
    {"path": 0, "Location_local_id": 17},
    {"path": "http://192.168.1.66/", "Location_local_id": 18},
]
...
def is_aruco_inside_box(aruco_corners, box):
    ...
    return (
        aruco_x >= box_x
        and aruco_y >= box_y
        and aruco_x + aruco_w <= box_x + box_w
        and aruco_y + aruco_h <= box_y + box_h
    )
...
def draw_text_with_background(
    img, text, org, font, font_scale, color, thickness=1, background_color=None
):
    ...
    box_coords = (
        (text_offset_x, text_offset_y + baseline - 2),
...
def main():
    ...
    for video in videos:
        try:
            ...
            # Captura do vídeo via HTTP (URL HTTP direto)
            cap = cv2.VideoCapture(video["path"])  # A URL do stream HTTP
            if not cap.isOpened():
                raise ValueError(f"Erro ao abrir o vídeo: {video['path']}")
            caps.append(cap)
        except Exception as e:
            print(f"Erro ao carregar vídeo ou modelo para {video['path']}: {e}")
            caps.append(None)
    ...
    while True:
        ...
        for i, cap in enumerate(caps):
            if cap is None:
                ...
                )  # Frame preto para indicar vídeo indisponível
                ...
                for j, corner in enumerate(corners):
                    for k, box in enumerate(all_boxes[i]):
                        if is_aruco_inside_box(corner, box):
                            ...
                            if (
                                last_sent_times[i] is None
                                or (current_time - last_sent_times[i]).total_seconds()
                                >= min_interval
                            ):
                                send_data_to_backend(data)
                                last_sent_times[i] = current_time
                            last_detected[i][aruco_id] = {
                                "time": current_time,
                                "box": box,
                            }
        ...
        for i, frame in enumerate(frames):
            draw_text_with_background(
                frame,
                f"Location ID: {videos[i]['Location_local_id']} - {current_time}",
                (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (255, 255, 255),
